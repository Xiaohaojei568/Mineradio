...
### 2026-07-01 - 保留 Home 施工区高度与搜索框间距

- 用户认可/要求保留：当前首页布局“很棒”；左侧施工区要和右侧整组内容等高，顶部搜索框和下方 Home 内容之间要有更舒服的呼吸感。
- 涉及文件：`public/index.html`、`docs/PROJECT_MEMORY.md`。
- 关键参数/实现：`.empty-home-shell` 使用 `grid-template-rows:auto auto`，避免右侧底部推荐行溢出后左侧施工区偏短；桌面 Home 起始位置使用 `body.desktop-shell #empty-home{top:150px}`，全屏使用 `top:clamp(112px,15vh,124px)`，矮窗口分别保留 `max-height:760px -> top:136px`、`max-height:700px -> top:120px`，窄屏桌面为 `top:148px`。
- 禁止回退或改坏的点：不要把 Home 网格改回 `auto 1fr` 导致左侧施工区短一截；不要把搜索框和首页卡片重新挤得太近；后续调整首页卡片尺寸时要同时检查桌面、最大化/全屏、矮窗口和窄屏下底部推荐卡片不被挤掉。
