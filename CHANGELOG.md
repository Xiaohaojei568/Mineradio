...
- 修正正式更新通道到 `Xiaohaojei568/Mineradio`，后续版本从自己的仓库检查和下载更新。
- 右上角更新入口改为常驻静态检查按钮：无更新时不呼吸、不高亮，有更新时再提示，减少空闲动画占用。
- 发布 `1.1.1 -> 1.1.4` 快速补丁，便于当前安装版直接验证轻量更新链路。

## v1.1.3

- 优化更新按钮动画，隐藏或无更新时不再保留呼吸动效，减少空闲动画占用。
- 优化气泡弹幕后台行为，应用隐藏、最小化或进入深度休眠时暂停弹幕计时器，回到前台后再恢复。
- 移除桌面歌词/壁纸覆盖层的重复固定同步兜底，保留渲染循环内的节流同步，降低常驻 CPU 开销。
