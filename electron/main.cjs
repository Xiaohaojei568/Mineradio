...
function logLocalServerStream(label, chunk) {
  const text = String(chunk || '').trim();
  if (!text) return;
  if (label === 'stderr' || isDesktopDebugMode()) {
    console[label === 'stderr' ? 'warn' : 'log'](`[LocalServer] ${text}`);
  }
}

async function startLocalServer(port) {
  const serverEntry = path.join(__dirname, '..', 'server.js');
  if (process.env.MINERADIO_EMBED_SERVER === '1') {
    localServer = require(serverEntry);
  ...
  const child = spawn(process.execPath, [serverEntry], {
    ...
    env: {
      ...
      HOST: '127.0.0.1',
      PORT: String(port),
      MINERADIO_PARENT_PID: String(process.pid),
    },
    windowsHide: true,
    stdio: ['ignore', 'pipe', 'pipe'],
  });
  localServer = child;
  child.stdout.on('data', chunk => logLocalServerStream('stdout', chunk));
  child.stderr.on('data', chunk => logLocalServerStream('stderr', chunk));
  child.on('exit', (code, signal) => {
    if (localServer === child) localServer = null;
    if (!appQuitting) {
      console.warn(`[LocalServer] exited unexpectedly (${code != null ? code : signal || 'unknown'})`);
    }
  });
  ...
  await new Promise((resolve, reject) => {
    ...
    const onExit = (code, signal) => done(new Error(`LOCAL_SERVER_EXITED_${code != null ? code : signal || 'UNKNOWN'}`));
    child.once('error', onError);
    child.once('exit', onExit);
    waitForLocalHttp(port, 15000).then(() => done()).catch(done);
  });
}
...
function configureAppPermissions() {
  try {
    ...
    console.warn('Permission configuration skipped:', e.message);
  }
}
...
async function createWindow() {
  ...
  windowFullscreenActive = false;
  const configuredApiPort = Number.parseInt(process.env.MINERADIO_API_PORT || process.env.PORT || '', 10);
  const port = configuredApiPort > 0 ? configuredApiPort : await findOpenPort(3000);
  mainServerPort = port;
...
if (!gotSingleInstanceLock) {
  ...
  app.on('before-quit', () => {
    ...
    else if (localServer && localServer.kill) localServer.kill();
  });
}
